<div class="row">
    <div class="col">
        <form class="d-inline" action="{{ route('admin.manage.baptis.img', ['id' => $model->id]) }}" method="GET">
            <button id="btn-view" class="btn btn-icon btn-3 btn-default btn-sm" type="submit">
                <span class="btn-inner--text">View</span>
            </button>
        </form>

        <form class="d-inline" method="post" action="{{ route('admin.manage.baptis.validate', ['id' => $model->id]) }}">
            @csrf
            @method('PUT')
        ...
        <form class="d-inline" method="post" action="{{ route('admin.manage.baptis.invalidate', ['id' => $model->id]) }}">
            @csrf
            @method('PUT')
